 ...
 * file, You can obtain one at http://mozilla.org/MPL/2.0/. */
 ...
 ///
 pub fn get_default_list() -> Option<Arc<TodoList>> {
 ...
 ///
 pub fn set_default_list(list: Arc<TodoList>) {
     *DEFAULT_LIST.write().unwrap() = Some(list);
 }
 
 /// Create a new TodoEntry from the given string.
 ///
 pub fn create_entry_with<S: Into<String>>(item: S) -> Result<TodoEntry> {
     let text = item.into();
     if text.is_empty() {
 ...
 impl TodoList {
     ...
     fn get_last_copy2(&self) -> Result<String> {
         let items = self.items.read().unwrap();
         items.last().cloned().ok_or(TodoError::EmptyTodoList)
     }
     ...
     fn get_items_copy(&self) -> Vec<String> {
        let items = self.items.read().unwrap();
        items.clone()
     }

     fn get_items_copy2(&self) -> Vec<String> {
        let items = self.items.read().unwrap();
        items.clone()
     }
